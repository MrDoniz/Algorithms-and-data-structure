...
public:
  ...
  void      add(const sparse_matrix_t&, matrix_t<double>&);
...
//Modificacion
void
sparse_matrix_t::add(const sparse_matrix_t& A, matrix_t<double>& B)
{
  ...
  for(int i = 0; i < get_m(); i++)
    for(int j = 0; j < get_n(); j++){
      if(j < A.get_n() || i < A.get_m())
        B(i+1,j+1) =  A.sm_.get_val(i).auxGetVal(j) + sm_.get_val(i).auxGetVal(j);
      else
        B(i+1,j+1) = 0 + sm_.get_val(i).auxGetVal(j);
    }
}

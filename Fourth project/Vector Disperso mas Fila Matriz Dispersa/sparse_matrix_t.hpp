...
public:
  ...
  void add(const sparse_vector_t&, int, vector_t<double>&);
  // E/S
...
// Modificacion
void
sparse_matrix_t::add(const sparse_vector_t& sv1, int i, vector_t<double>& v2){
  assert(sv1.get_n() == get_n());
  v2.resize(get_n());
  for(int j = 0; j < get_n(); j++){
    v2(i) = sv1.auxGetVal(i) + v2.set_val(j, sv1.auxGetVal(j));
  }
}
